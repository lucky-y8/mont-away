...
function MapView({ t, route, routeMode = null, onPick, onPointMove, pickHint = '', showLocate = false }) {
  ...
  if (amapConfigured && !amapFailed) return <AmapRouteMap points={points} loadingText={t.mapLoading} pickHint={pickHint} routeMode={hasRecordedTrack ? null : routeMode} routeReadyText={t.roadRouteReady} routeFallbackText={t.roadRouteFallback} locateText={showLocate ? t.useCurrentLocation : ''} locatingText={t.locatingCurrent} locationErrorText={t.currentLocationError} markerStart={t.start} markerEnd={t.end} showPointMarkers={!hasRecordedTrack} onPick={onPick} onPointMove={onPointMove} onError={handleAmapError}/>
  const fallbackPoints = points.slice(0, 4)
  return <div className="map-view"><div className="lake"/><div className="track"/>{fallbackPoints.map((point, index) => <button type="button" title={point.name} className={'pin p' + index} key={`${point.name}-${index}`}><span>{index === 0 ? t.start : index === fallbackPoints.length - 1 ? t.end : index}</span></button>)}<small>{t.mapSample}</small></div>
}

function PostCard({ initialPost, openRoute, openPlace, t, locale, user, onRequireAuth }) {
...
function RankingPage({ t, locale, openRoute, openPlace, user, onRequireAuth }) {
  const [posts, setPosts] = useState(null)
  const [notice, setNotice] = useState('')
...
function PlacePage({ detail, error, openRoute, openPlace, t, locale, user, onRequireAuth }) {
  if (error) return <div className="surface empty"><h1>{t.placePageTitle}</h1><p>{error}</p></div>
  if (!detail) return <div className="feed"><p className="feed-status">{t.loadingFeed}</p></div>
...
function PostDetailPage({ post, error, openRoute, openPlace, t, locale, user, onRequireAuth }) {
  ...
  return <div className="feed post-detail-page"><PostCard initialPost={post} openRoute={openRoute} openPlace={openPlace} t={t} locale={locale} user={user} onRequireAuth={onRequireAuth}/></div>
}

function TrendingRail({ t, locale, openPlace, onOpenRanking }) {
  const [posts, setPosts] = useState(null)
...
function locationState() {
  ...
  // Keep public place pages shareable and browser-history aware. / 景点主页支持分享链接与浏览器前进后退。
  const placeMatch = path.match(/^\/places\/([^/]+)\/?$/)
  if (placeMatch) return { page: 'place', placeId: placeMatch[1] }
...
function App() {
  ...
  useEffect(() => {
    const applyLocation = () => {
      ...
      if (next.placeId) {
        ...
        setPlaceError('')
        getPlace(next.placeId, locale).then(setSelectedPlace).catch(error => setPlaceError(error.message))
      }
    }
    ...
    if (initialLocation.placeId) getPlace(initialLocation.placeId, locale).then(setSelectedPlace).catch(error => setPlaceError(error.message))
  ...
  const openPost = post => { setSelectedPost(post); setPostError(''); go('post', `/posts/${post.id}`) }
  // Fetch a fresh aggregate so bookmark state follows the current viewer. / 每次进入都刷新聚合数据，确保收藏状态属于当前用户。
  const openPlace = place => {
    setSelectedPlace(null)
    setPlaceError('')
    go('place', `/places/${place.id}`)
  ...
  const content = page === 'home'
    ...
    : page === 'map' ? <Discover openPlace={openPlace} t={t} locale={locale}/>
      : page === 'route' ? postError ? <PostDetailPage error={postError} t={t}/> : selectedPost ? <RoutePage t={t} post={selectedPost} openPlace={openPlace}/> : <PostDetailPage t={t}/>
        : page === 'post' ? <PostDetailPage post={selectedPost} error={postError} openRoute={openRoute} openPlace={openPlace} t={t} locale={locale} user={user} onRequireAuth={requireAuth}/>
          : page === 'place' ? <PlacePage detail={selectedPlace} error={placeError} openRoute={openRoute} openPlace={openPlace} t={t} locale={locale} user={user} onRequireAuth={requireAuth}/>
