...
function GiftPage({ t, locale, user, onRequireAuth }) {
  ...
  const submit = async event => {
    ...
    const data = new FormData(event.currentTarget)
    try {
      await redeemGift(selected.id, { quantity: Number(data.get('quantity')), recipient_name: data.get('recipientName'), contact: data.get('contact'), shipping_address: data.get('shippingAddress') }, locale)
      setSelected(null)
      setNotice('')
      await load()
...
function AdminPage({ t, locale, openRoute }) {
  ...
  const addGift = async event => {
    ...
    const payload = {
      ...
      point_cost: Number(data.get('pointCost')), stock: Number(data.get('stock')), is_active: true,
    }
    try { await createGift(payload, locale); event.currentTarget.reset(); setNotice(''); load() } catch (error) { setNotice(error.message) }
  }
